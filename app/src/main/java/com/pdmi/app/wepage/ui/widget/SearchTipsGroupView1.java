...
public class SearchTipsGroupView1 extends LinearLayout {

    private Context context;

    public SearchTipsGroupView1(Context context) {
        super(context);
        this.context = context;
        setOrientation(VERTICAL);//设置方向
    }

    public SearchTipsGroupView1(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
        setOrientation(VERTICAL);//设置方向
    }

    public SearchTipsGroupView1(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void initViews(List<StudioItem> items, final OnItemClick onItemClick) {
        ...
        for (int i = 0; i < size; i++) {//便利items
            ...
            itemLp.leftMargin = 55;
            itemLp.height =((int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 36, getResources().getDisplayMetrics()));

            //得到当前行的长度
            length += 55 + getViewWidth(itemView);
            if (length + 55 > screenWidth) {//当前行的长度大于屏幕宽度则换行
                length = 0;
